...
generated from a compiled build of that file via ctypes (see
`reference/README.md`). **Devillers-Pion 2003** describes a refinement
...
- **Shewchuk, J. R.**
  *predicates.c* — the public-domain reference C source.
  ...
  This file is the canonical oracle. The golden master generator
  (`golden/generate.py`) compiles and links against it via ctypes; the
  Python `predicates_reference.py` is cross-validated against it on
  every test case before any expected output is committed.

...
**Recommended next pickup: `hv0.11`** — `bench/meijer-g/` full golden battery against `tools/meijer-g`. With Layer 7 shipped, the next step is to run the problem-13 verifier against ~150 cases generated via mpmath / Wolfram triple-witness. Every case lands in one of three honest output shapes (symbolic AST / numerical record / tagged refusal); the verifier compares numerical outputs to oracle truths within `1e-(precision − 5)` (Tier C) or `1e-(precision − 8)` (Tier D/E), and refusal-class outputs against the expected refusal-tag. Sibling of `bench/hypergeometric-pfq` (`hv0.4` ✓, prior session) — same tier-graded JSONL manifest + in-process compose runner + mpmath/Wolfram cross-validation + ≥ 4 invariant checks per case + 5+ mutation-prove perturbations.

Alternative: follow-up beads to complete the Braaksma theorem (`hv0.9.1`–`hv0.9.5`: full H_{p,q} algebraic series, Stokes-line connection coefficients, hyperasymptotic refinement, symmetric `|z| → 0`, secondary-sector handling), or the contour-layer cost-bound truncation cap (filed in this session — the dispatcher pre-filter shadows the issue today), or the `lc1` runner-side `--precision` flag wiring (affects every arbprec tool's CLI invocation; in-process callers via `@workbench/compose` are unaffected), or the two follow-up beads from `hv0.4` (P1 compose `--precision`-merge for arbprec tools — the same as `lc1`; P2 analytic continuation for `|z| ≥ 0.99`).

(`hv0.10` shipped this session — see `scientist-workbench` worklog 079.)

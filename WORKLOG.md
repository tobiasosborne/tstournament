...
Last updated: 2026-05-08, end of Opus 4.7 (1M) session 3 — **substrate `exp()` "P1 regression" was a false alarm; closed as such, with surgical principled hardening applied.** Substrate verified byte-identical to mpmath at every tested digit (14 inputs × 5 precisions, 30–1000 dps). The bead `4ne`'s empirical accuracy table was generated against bogus reference values, not against any cited oracle. Slater identity-test thresholds remain at the *algorithm's* honest ulp budget (Γ-products + residue summation + prefactor), not a substrate cap. See `scientist-workbench` worklog 071 for the full diagnosis + comparison vs. an Opus subagent's from-scratch redesign. Campaign still at **3 of 12** child beads closed (`hv0.1`, `hv0.3`, `hv0.5`); recommended next: `hv0.7` (arb-prec quadrature).
...
**Recommended next pickup: `hv0.7`** — generalise `packages/quadrature` to arbitrary precision (needed by `hv0.8` Mellin-Barnes contour). Has no upstream dependencies beyond `hv0.1` ✓.

(`scientist-workbench-4ne`, the "bigfloat `exp` precision regression" filed in session 2, was closed as a false alarm in session 3. The substrate is byte-identical to mpmath at every tested digit. See `scientist-workbench` worklog 071 for the full diagnosis. The shard `070-meijer-core-slater.md` carries a top-of-section retraction.)

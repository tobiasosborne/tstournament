...
Last updated: 2026-05-09, end of Opus 4.7 (1M) session 9 — **`hv0.4` `bench/hypergeometric-pfq` tier-graded battery shipped** (`bench/hypergeometric-pfq/` ~1100 LOC across `DESCRIPTION.md` / `PROMPT.md` / `REFERENCES.md` / `golden/{verify.py, verifier_protocol.md, test_mutations.py}` / `reference/generate-mpmath-truth.py` / `run-candidate.ts`; worklog 078). Validation surface for the inner `pFq` path that Slater (`packages/meijer-core::Slater`), Braaksma asymptotic (`hv0.9` ✓), and the upcoming top-level dispatcher (`hv0.10`) all consume. **53 cases × ~5 invariant checks ≈ 282 assertions across 6 tiers** per Pearson-Olver-Porter 2017 §3 four-failure-mode taxonomy: **Tier 0** closed-form anchors (`exp / log / sin / cos / sinc / (1−z)^{−a}`, 11 cases), **Tier A** generic happy path (15 cases incl. precision=100 stress), **Tier B** large parameters (`a, b ∈ [10, 100]`, 10 cases), **Tier C** near-unit-circle (`|z| ∈ [0.85, 0.98]`, 8 cases), **Tier D** parameter coalescence (5 cases), **Tier E** structural refusals (4 cases incl. famous `2F1(1, 1; 2; -1) = log 2` boundary). Cross-validated mpmath at 80 dps + Wolfram at `precision + 30` dps — 49 of 49 numerical cases agree, 4 structural refusals pinned. Five mutation-prove RED tests per ADR-0019 §4 / CLAUDE.md Rule 6 (doubled truth, value-for-tagged swap, 1e-30 perturbation under 1e-42 tolerance, over-reported precision, unknown method). `bun run check` green. Two follow-up beads filed for the orchestrator: P1 `compose.runWorkbench` doesn't merge ADR-0020's standard `--precision` flag for arbprec tools (the bench routes via `executeToolDef` directly as documented workaround — same flag-wiring gap surfaced session 8); P2 analytic-continuation path for `|z| ≥ 0.99` to reclaim the famous Abel-summation identity (currently in Tier E refusal). Campaign now at **9 of 12** child beads closed (`hv0.1`, `hv0.2`, `hv0.3`, `hv0.4`, `hv0.5`, `hv0.6`, `hv0.7`, `hv0.8`, `hv0.9`); recommended next: `hv0.10` — the top-level `tools/meijer-g` dispatcher composing the four algorithmic layers — every dependency is now in place, and the `bench/hypergeometric-pfq` validation surface is the discrimination floor for the dispatcher's pFq lane.
...
**Recommended next pickup: `hv0.10`** — the top-level `tools/meijer-g` dispatcher. Composes the four algorithmic layers (`hv0.5` Slater + `hv0.6` symbolic + `hv0.8` contour + `hv0.9` ✓ asymptotic) into a single user-facing tool: symbolic-first → Slater → contour → asymptotic-or-refuse. **Every layer it depends on is now shipped** — `meijergSlater`, `meijergSymbolic`, `meijergContour`, and `meijergAsymptotic` all expose mirrored result envelopes so the dispatcher is one switch statement. The integration test of the seven-layer stack: once it lands, problem-13 Tiers A/B (symbolic) and C/D/E/G (numerical + refusal) all flow through one entry point. The `bench/hypergeometric-pfq` validation surface (`hv0.4` ✓, this session) is the discrimination floor for the inner pFq path the dispatcher's Slater and asymptotic lanes consume.

Alternative: follow-up beads to complete the Braaksma theorem (`hv0.9.1`–`hv0.9.5`: full H_{p,q} algebraic series, Stokes-line connection coefficients, hyperasymptotic refinement, symmetric `|z| → 0`, secondary-sector handling), or to lift the `hv0.8` contour layer's ~22-dps achievable ceiling, or the two follow-up beads from `hv0.4` (P1 compose `--precision`-merge for arbprec tools; P2 analytic continuation for `|z| ≥ 0.99`).

(`hv0.4` shipped this session — see `scientist-workbench` worklog 078.)

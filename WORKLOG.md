...
Last updated: 2026-05-09, end of Opus 4.7 (1M) session 8 — **`hv0.9` Braaksma far-field asymptotic shipped (v0.1)** (`packages/meijer-core/src/asymptotic.ts` ~470 LOC + ~600 LOC test surface across `asymptotic.test.ts` and `asymptotic-mutations.test.ts`; ADR-0026; worklog 077). Layer 6 of the seven-layer Meijer G stack — the far-field path for `|z| → ∞`. v0.1 ships the **algebraic dominant asymptotic** in the principal sector `|arg z| < π/2 − π/64`: the n-pole Slater Series 2 read asymptotically and truncated at its **optimal** index per Olver 1974 §3.7 "superasymptotic" (stop when `|t_{k+1}| ≥ |t_k|`; report `|t_{k*+1}|` as error estimate). Cross-validated against `mpmath.meijerg` at 80 dps (5 cases) and `wolframscript MeijerG` at 60 dps (5 cases); method-agreement with `meijergSlater` on overlap region tested at 30 dps and confirmed to ~25 dps. 5 mutation-prove tests (sign, prefactor, truncation, sector classifier, 1/z-omission) catch the four major regression classes. Structured refusal envelope (`stokes-line`, `secondary-sector`, `small-z`, `non-asymptotic-regime`, `no-pole-residues`, `input-error`) keeps wrong-valued answers out of out-of-scope sectors; full Braaksma theorem (E-series + Stokes-multiplier table + secondary-sector connection coefficients + Olde Daalhuis–Olver hyperasymptotic refinement + symmetric `|z| → 0` asymptotic) deferred to follow-up beads `hv0.9.1`–`hv0.9.5`. Wire wrapper `tools/meijer-g-asymptotic-only` (7 tests + 8 goldens; `arbprec: true` with the standard `--precision=N` flag). Result envelope mirrors `MeijerGContourResult` line-for-line on the common fields so the hv0.10 dispatcher's compose code is one switch statement. Pre-existing `--precision` flag wiring gap diagnosed (the runner parses but doesn't thread the flag into arbprec tools' `fn` flags; affects `hypergeometric-pfq`, `meijer-g-slater-only`, `meijer-g-asymptotic-only` consistently); follow-up bead filed. Campaign now at **8 of 12** child beads closed (`hv0.1`, `hv0.2`, `hv0.3`, `hv0.5`, `hv0.6`, `hv0.7`, `hv0.8`, `hv0.9`); recommended next: `hv0.10` — the top-level `tools/meijer-g` dispatcher composing the four algorithmic layers — every dependency is now in place.

...
**Recommended next pickup: `hv0.12`** — tstournament problem-13 staging, the **final ship** of the campaign. With `hv0.11` closing the bench validation surface (91/91 green, ~434 invariant assertions across nine tiers, two-oracle consensus + Tier-0 RHS-evaluated anchors), the campaign now has all algorithmic pieces in place (Layers 1–7) plus its own golden battery. Stage the tstournament `problems/13-meijer-g/golden/` directory from `bench/meijer-g/golden/` (rename + adapt the wire format to the `tstournament` JSONL convention from VERIFIER-PROTOCOL.md), publish the prompt as a Phase-3 trial, and run an Opus 4.7 (1M) baseline.

Alternative: address the four follow-up beads filed by `hv0.11` (P1 dispatcher precision over-reporting, P1 3+-pole coalescence hang, P2 large-|z| Slater ceiling, P2 rational-real symbolic admission) — any one of them tightens the bench's tolerances and broadens its coverage.  Or follow-up beads to complete the Braaksma theorem (`hv0.9.1`–`hv0.9.5`: full H_{p,q} algebraic series, Stokes-line connection coefficients, hyperasymptotic refinement, symmetric `|z| → 0`, secondary-sector handling), or the contour-layer cost-bound truncation cap, or the `lc1` runner-side `--precision` flag wiring (affects every arbprec tool's CLI invocation; in-process callers via `@workbench/compose` are unaffected), or the two follow-up beads from `hv0.4` (P1 compose `--precision`-merge for arbprec tools — the same as `lc1`; P2 analytic continuation for `|z| ≥ 0.99`).

(`hv0.11` shipped this session — see `scientist-workbench` worklog 081.)

...
Last updated: 2026-05-08, end of Opus 4.7 (1M) session 6 — **`hv0.2` cas-core special-function AST vocabulary extension shipped** (`packages/cas-core/src/special-functions.ts` ~440 LOC; surgical edits to `diff.ts`; 45 new tests in `special-functions.test.ts` + 14 new cas-diff goldens; ADR-0023). 27-head closed vocabulary covering Gamma family, Bessel J/Y/I/K, HypergeometricPFQ, Whittaker, ParabolicCylinderD, Erf/Erfc, ExpIntegralEi/E, Fresnel, Legendre, classical orthogonal polynomials (Laguerre, Hermite, Chebyshev T/U, Gegenbauer), Polylog, LerchPhi, MeijerG. Differentiable subset of 15 heads ships DLMF-cited closed-form rules (Gamma family, Erf/Erfc, ExpIntegralEi/E, Fresnel, Bessel J/Y/I/K, HermiteH, Polylog); deferred subset (HypergeometricPFQ, MeijerG, Whittaker, ParabolicCylinder, Legendre P/Q, LaguerreL, Chebyshev T/U, GegenbauerC, LerchPhi) refuses honestly via the existing `tagged "cas-diff/out-of-scope"` boundary. Wolfram convention for list-parameter heads: `HypergeometricPFQ` is `(list, list, scalar)`, `MeijerG` is `(list-of-list, list-of-list, scalar)`. Smart constructors exported (not duplicated) so the dispatcher reuses `diff.ts`'s `mkPlus / mkMinus / mkNeg / mkTimes / mkDiv / mkPower`. Discrete-order parameters (n, ν, s) refuse honestly when wrt depends on them — closed-form ∂/∂ν J_ν exists (involves Polygamma) but is deferred. Campaign now at **6 of 12** child beads closed (`hv0.1`, `hv0.2`, `hv0.3`, `hv0.5`, `hv0.7`, `hv0.8`); recommended next: `hv0.6` (Adamchik–Marichev + Roach symbolic dispatch — load-bearing for problem-13 Tiers A/B) or `hv0.9` (Braaksma asymptotic) — both unblocked.

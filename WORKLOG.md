...
Last updated: 2026-05-08, end of Opus 4.7 (1M) session 4 — **`hv0.7` arb-prec quadrature substrate shipped** (`packages/quadrature/src/{nodes-weights-bf,gauss-kronrod-bf}.ts`, ~720 LOC, 72 tests). New `gaussKronrodAdaptiveBF` driver carries the load-bearing **centred-delta K-G identity** that eliminates cancellation at high working precision (the naive `K - G` cancellation chews through 50–70 bits at 200+ bit precision; the algebraic identity `K - G = Σ_i (WGK_i - WG_i) · (fSum_i - 2·f(centre))` makes the centre's contribution vanish and the remaining sum well-conditioned by construction — see ADR-0021 for the derivation). Also: heap-rebuild over delta-update for the running totals (the float64 driver's `total += new - old` chews through lower bits at high precision); Cauchy value-stability secondary convergence at substrate ulp + 30-bit headroom (K-G measures G7's algebraic error which floors above the user tol on polynomial / smooth-analytic integrands long before K15's actual error reaches the floor — the substrate-ulp Cauchy criterion declares convergence honestly). 200-dps node/weight constants generated via mpmath at 230 dps using Stieltjes orthogonality (Patterson 1968 / Laurie 1997); cross-verified against GSL `qk15.c` byte-for-byte at the truncated precision. Cap: `MAX_DECIMAL_PRECISION = 150`. New ADR-0021 documents the layering (library extension only — no new wire tool yet; consumer is `hv0.8` Mellin-Barnes which is in-process). Campaign now at **4 of 12** child beads closed (`hv0.1`, `hv0.3`, `hv0.5`, `hv0.7`); recommended next: `hv0.8` (Mellin-Barnes contour quadrature).
...
**Recommended next pickup: `hv0.8`** — Mellin-Barnes contour quadrature in `@workbench/meijer-core`. The arb-prec quadrature substrate (`gaussKronrodAdaptiveBF`) shipped in `hv0.7` (this session) is the in-process surface this consumer needs. Natural design question: the BigComplex codomain extension. The contour integrand is complex-valued (`Γ`-product times `z^s` along a vertical line); `hv0.8` picks per-component split (run the real driver twice on Re(f) and Im(f) with caching) vs codomain-generic helper. ADR-0021 §"What we will not decide" flags this as deferred; the right shape is `hv0.8`'s call.

(`hv0.7` shipped this session — see `scientist-workbench` worklog 072 and ADR-0021. `scientist-workbench-4ne`, the "bigfloat `exp` precision regression" filed in session 2, was closed as a false alarm in session 3. The substrate is byte-identical to mpmath at every tested digit. See `scientist-workbench` worklog 071 for the diagnosis.)

...
Last updated: 2026-05-08, end of Opus 4.7 (1M) session 7 — **`hv0.6` Adamchik–Marichev + Roach symbolic dispatch shipped** (`packages/meijer-core/src/{dispatch.ts,dispatch-types.ts,dispatch-rules/{bateman-5-6,dlmf-16-18}.ts}` ~700 LOC + ~330 LOC test surface; ADR-0025; worklog 076). Pattern-table dispatcher of curated reduction rules from primary literature: ≥30 starter rules from Bateman §5.6 + DLMF §16.17–§16.18, every rule cross-checked against `mpmath.meijerg` at 30 dps before inclusion. First-match-wins over `ALL_RULES`; rules organised one-file-per-source under `dispatch-rules/`. Inputs canonicalised by sub-tuple-internal canonical-bytes sort (rationals before integers, integers before symbols — surprise: `canonicalize` sorts JSON record keys alphabetically, so `rational` Values starting with `{"d` sort before `integer` Values starting with `{"k`). Outputs canonicalised through `casSimplify`. No-known-reduction envelope per ADR-0003 boundary-failure. Audit grep test enforces no transliteration from open-source implementation source. Wire wrapper `tools/meijer-g-symbolic-only` (7 tests + 13 goldens). Smart constructors `mkPlus / mkMinus / mkNeg / mkTimes / mkDiv / mkPower` lifted from cas-core internal to public surface for dispatch-layer reuse. Bulk-of-corpus follow-up beads filed (`hv0.6.1` PBM Vol 3 §8.4 ~600 rules; `hv0.6.2` Mathai 1993; `hv0.6.3` Wolfram Functions Site shards by family; `hv0.6.4` argument-transformation infrastructure; `hv0.6.5` richer pattern grammar). Campaign now at **7 of 12** child beads closed (`hv0.1`, `hv0.2`, `hv0.3`, `hv0.5`, `hv0.6`, `hv0.7`, `hv0.8`); recommended next: `hv0.10` (top-level `tools/meijer-g` dispatcher — composes the four algorithmic layers into one user-facing tool) or `hv0.9` (Braaksma asymptotic) — both unblocked.
...
**Recommended next pickup: `hv0.2`** — cas-core special-function AST extension. The next prerequisite for both `hv0.6` (Adamchik-Marichev + Roach symbolic dispatch) and `hv0.9` (Braaksma asymptotic + hyperasymptotic layer); unblocks the most downstream work in the campaign DAG. The extension adds Bessel/Whittaker/PFQ/Erf/Gamma/Digamma/Polygamma/MeijerG heads to the `cas-core` AST plus their `cas-diff` derivative rules — see `ts-bench-infra/problems/13-meijer-g/PLAN.md` Layer 1 for the full spec.

Alternative: `hv0.4` (bench/hypergeometric-pfq tier-graded battery), or follow-up beads to lift the `hv0.8` contour layer's ~22-dps achievable ceiling (widen `cgamma`'s Stirling budget or compute the integrand via `clgamma` directly to avoid Γ-product cancellation), or to add asymmetric contour offset for non-real `arg(z)`.

(`hv0.8` shipped this session — see `scientist-workbench` worklog 073 and ADR-0022.)
